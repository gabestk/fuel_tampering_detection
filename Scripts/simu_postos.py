    ...
    try:
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            for vehicle_id in traci.vehicle.getIDList():
                ...
                vehicle_factory_error = np.random.normal(0, (error_percentage))
                vehicle_random_error = np.random.normal(vehicle_factory_error, (error_percentage))
                ...
                if checkStation(vehicle_id, combus_percentage):
                    ...
                    if vehicle_info[vehicle_id]["fueling"] == True:
                        ...
                        vehicle_random_error = np.random.normal(vehicle_factory_error, (error_percentage))
                        random_refuel = random.uniform(10, 20) / 100 * tanque
                        if pump == 1:
                        ...
                        if len(erros_station) >= 2:
                            station_mean_error = sum(erros_station)/len(erros_station)    
                            incerteza = calcular_incerteza_erro_padrao(erros_station)
                            station_uncertainty.append(incerteza)

                        if len(erros_station) >= 20:
                            station_mean_error = sum(erros_station[-20:])/20    
                            incerteza = calcular_incerteza_erro_padrao(erros_station)
                            station_uncertainty.append(incerteza)


                        if len(station_uncertainty) >= 10:
                            #for j in range(len(uncertainty) - 4):
                            station_janela = station_uncertainty[len(station_uncertainty) - 10 : len(station_uncertainty)]
                            if max(station_janela) - min(station_janela) < 0.5:
...
for i in range(1, 11):
    if __name__ == "__main__":
        ...
        traci.start([sumoBinary, "-c", "Sumo/osm.sumocfg", "--tripinfo-output",
                    "Sumo/tripinfo.xml", "--max-num-vehicles", "100", "--no-warnings", "--quit-on-end"])
